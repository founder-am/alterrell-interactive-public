/* S: final capture, scroll-behavior disabled. Uncommitted byproduct. */
...
(async()=>{
  ...
  for (const j of jobs) {
    ...
    console.log(`${j.tab}  ${j.sel}`);
    console.log(`   rect ${chk.w} x ${chk.h}   top ${chk.top}  bottom ${chk.bottom}`);
    console.log(`   scroll-behavior: ${chk.sb}   non-zero: ${chk.w>0&&chk.h>0}   intersects viewport: ${chk.intersects}`);
    if(!chk.w||!chk.h||!chk.intersects){console.log('   STOP: zero-size or off-viewport, refusing to write a blank frame');bad++;await ctx.close();continue;}
