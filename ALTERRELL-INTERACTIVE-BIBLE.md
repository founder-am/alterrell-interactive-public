...
| TABS-02 | Tab order, counting from the last tab (position *n*): *n* = Sources, *n*−1 = Share to Social, *n*−2 = Take Action. "Spread the Word" is retired as a tab name for pieces built or rebuilt 2026-07 forward (per D-109's forward-only default). Concert Tax and Sodium are named exceptions — they grandfather and keep "Spread the Word." | D-102; D-109 | Last three tabs, in order, are not [Take Action, Share to Social, Sources] for a piece built/rebuilt 2026-07 forward; or a piece other than Concert Tax or Sodium uses the label "Spread the Word." |
...
| TABS-06 | No sub-tabs (a `role="tablist"` nested inside a `role="tabpanel"`). Audience segmentation is carried in the card eyebrow, not a second tab layer. Binds pieces built or rebuilt 2026-07 forward (D-109 default — no explicit grandfather stated in D-98 itself). **Known current violation, not yet grandfathered clean:** the live `gay-uncles/index.html` (`.bbl-subtabs`, "Audience navigation" — For the Big Men / For the Chasers / For the Friend Group) has sub-tabs, and so do *both* in-flight rebuild drafts (`gay-uncles/index.new.html` and `gay-uncles/index.new-2026-07-21.html`). The live file predates 2026-07-01 and is grandfathered by D-109's default — but the rebuild drafts are not: whichever draft ships as the new `index.html` must remove the sub-tab pattern before it swaps in, or it ships in violation of its own build's ruling. | D-98; D-109 | A tabpanel contains a nested `role="tablist"`/`role="tab"` element set, in a piece built or rebuilt 2026-07 forward. |
| TABS-07 | RETIRED 2026-09-17. The Naming-series Sources-tab exemption is spent: every built Naming part (0, 3, 4, 5, 6) now carries a Sources tab, and parts 0, 1 and 2 were retrofitted onto the live-piece spine on 2026-09-17, so the series no longer runs a separate structure. No Naming exception applies. Parts 1 and 2 carry no Sources tab and are scored as failing like any other piece. | DDL D-71 (superseded); D-74; D-88; D-90; retrofit run 2026-09-17 | Any piece, Naming included, lacks a Sources tab at position *n*. |
...
| ID | RULE | SOURCE | FAILS IF |
|---|---|---|---|
| PROSE-01 | No prose width cap on any piece built or rebuilt 2026-07 forward. Every such piece wraps tab content in a padding-only container, full editorial width identical to the hero. Pieces already live as of 2026-07-01 keep their existing prose caps as a standing grandfather — this now explicitly includes Naming Part 3, not just Parts 0–2. Width and height caps apply to cards only (CARD-01), never as a *new* constraint on prose. | D-100; D-73 (grandfather clause stands, extended); D-109 | A piece built or rebuilt 2026-07 forward has a prose container (`.ai-inner` or a piece-local equivalent) with an explicit `max-width` below its natural container width. |
| PROSE-02 | Prose width is visually consistent across every tab of a piece. | DC Zone 6 | Not machine-checkable — visual verification only. *(For a piece bound by PROSE-01's no-cap rule, PROSE-01 gives this a checkable proxy: no tab-content container has a max-width. For a grandfathered piece, its existing cap should still apply uniformly across all of that piece's own tabs — inconsistency between tabs *within* a grandfathered piece is still a fail.)* |
...
| SHARE-02 | The Overview share block carries exactly 2 destinations: X and Copy Link. Binds pieces built or rebuilt 2026-07 forward (D-109 default — D-104 states no explicit grandfather, but in practice every checked live piece already implements exactly these 2 destinations, so no live piece currently needs the grandfather). | D-104 (retires the 6-destination rule); D-109 | Overview share block has a destination count ≠ 2, or a destination other than X/Copy Link, in a piece built or rebuilt 2026-07 forward. |
| SHARE-03 | The action-tab-minus-one tab (position *n*−1) is "Share to Social" (see TABS-02) for pieces built/rebuilt 2026-07 forward; Concert Tax and Sodium keep "Spread the Word" (TABS-02 grandfather). This tab contains, in order: intro copy → shareable cards → share destination grid → platform CTAs. No journey block on this tab. | DDL T-8 (renamed by D-102); D-109 | Tab at position *n*−1 is missing one of these zones, or contains a `.ai-journey` element. |
...
| ID | RULE | SOURCE | FAILS IF |
|---|---|---|---|
| SRC-01 | The Sources tab is visible — never a footer accordion — as the universal default, at position *n* (see TABS-02). No exceptions; the Naming pre-rebuild exemption was retired 2026-09-17 (TABS-07). | AMR §5; DC; D-102; retrofit run 2026-09-17 | Last tab's label is not "Sources". |
...
| ID | RULE | SOURCE | FAILS IF |
|---|---|---|---|
| FOOT-01 | Footer: wordmark + "← All pieces" link + copyright only. No tagline. No journey block. No methodology accordion. Dark background. | AMR §5; DC; DDL T-9; retrofit run 2026-09-17 | Footer contains a `.ai-journey` element, a `<details>` methodology block, or tagline text. The Naming-series footer methodology exception was retired 2026-09-17: parts 0, 1 and 2 now carry the spine footer and their methodology accordion was moved into the last tab panel. |
...
# Appendix A — Decision History (D-1 through D-109)
...
| D-73 | `.ai-inner` retired; padding-only prose containers; Parts 0–2 grandfathered. | PROSE — grandfather clause reaffirmed and extended to Part 3 by D-100 |
...
| D-96 | AMENDED. Gay Uncles is dark lane. Cards are 300×280 — dimension half of the original D-96 reversed by D-99. | CARD |
| D-97 | Gay Uncles ships 7 tabs. Hold Your Ground is cut. | TABS |
| D-98 | No sub-tabs. Audience segmentation lives in the card eyebrow. Sub-tabs exist in the live Gay Uncles build, not only in the draft — known unresolved violation, see Appendix C, C-m. | TABS |
| D-99 | Cards are 300×280 by default. Dimension exceptions apply only to pieces shipped before July 2026. | CARD |
| D-100 | No prose width cap on pieces built/rebuilt July 2026 forward. Pieces live as of July 1, 2026 keep existing prose caps. Width/height caps apply to cards only. D-73's grandfather clause STANDS, extended to Naming Part 3. | PROSE |
| D-101 | Tab count is what the piece needs. Under 7 preferred; 7+ needs logged AMA confirmation. Binds July 2026 forward. | TABS |
| D-102 | Tab order: *n*=Sources, *n*−1=Share to Social, *n*−2=Take Action, position 1=Overview. "Spread the Word" retired as a name going forward. Concert Tax and Sodium grandfather and keep it. | TABS |
...
- D-109, forward-only default: every ruling in this Bible applies from 2026-07-01 forward unless it states otherwise; a piece already live before that date grandfathers by default. This is the interpretive rule that governs how to read D-93 through D-108 (and every future ruling) when a rule's own text is silent on scope — not a condition to run against a single piece file. Most rules in this pass state their own scope explicitly (D-97, D-99, D-101, D-102, D-103, D-105, D-108); where one doesn't (D-93, D-98, D-104), this default supplies it, and the relevant main-body rows above note that explicitly. (D-109)
...
# Appendix C — Conflict Register, with Resolutions

C-a through C-l carry forward the Phase 1 conflict register. C-m is a new finding surfaced while writing this Bible's TABS section (D-98).
...
| C-f | Prose-width max-widths existed inconsistently: `.ai-inner` (platform CSS) carries none (compliant with D-73 for new pieces); Naming Parts 0–3 each define a piece-local `.win-inner { max-width: 860px }` — the figure D-73 rejected for pieces built forward. D-73's own text grandfathered Parts 0–2 by name but not Part 3, leaving Part 3's 860px cap unauthorized by any document. | Resolved by D-100: no prose width cap on any piece built or rebuilt 2026-07 forward; pieces already live as of 2026-07-01 keep their existing caps. D-73's grandfather clause **stands** and is explicitly **extended to Naming Part 3** — all four Naming parts (0–3) are now uniformly grandfathered, and none of their `.win-inner` 860px caps is a violation. Each part's cap is queued for removal only when that part is rebuilt per D-88/D-90, at which point PROSE-01's forward rule applies to it directly. |
...
| C-m | D-98 (no sub-tabs) has a live, current counter-example: `gay-uncles/index.html` ships a `.bbl-subtabs` "Audience navigation" block (For the Big Men / For the Chasers / For the Friend Group) — a nested `role="tablist"` inside the Overview tabpanel. Both in-flight rebuild drafts, `gay-uncles/index.new.html` (current WIP, contains the "Hold Your Ground" content D-97 cuts) and `gay-uncles/index.new-2026-07-21.html` (older snapshot), still carry the same `.bbl-subtabs` pattern — the rebuild has not yet removed it. | The live file predates 2026-07-01 and is grandfathered clean by D-109's default; it is not in violation. Neither draft is grandfathered, since a rebuild is by definition happening 2026-07 forward. Whichever draft becomes the new `index.html` must drop `.bbl-subtabs` and move audience segmentation into the card eyebrow (per D-98) before that swap — flagged here as an open item for the Gay Uncles rebuild, not resolved by this documentation-only pass. |
