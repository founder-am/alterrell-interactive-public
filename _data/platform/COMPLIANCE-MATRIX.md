...
| Item | Status | Notes |
|---|---|---|
| Tab bar alignment | ✅ | Uses `#ct-tabs` ID override — redundant after Batch A. Remove in Type 2. |
| Journey block | ⚠️ HTML only | Uses piece-local `.ct-journey`. CSS class `.ai-journey-compact` now exists — swap HTML in Type 2. |
...
| Item | Status | Notes |
|---|---|---|
| Tab bar alignment | ✅ | No piece-local override — inherits global `.ai-tabs`. After Batch A, now gets correct padding. |
| Journey block | 🔲 | No compact journey bar present at all. Add in Type 2. |
| Footer sources | ⚠️ inline | Uses raw `<details>` with all inline styles + `<ol>`. No CSS classes. Convert to `.ai-footer-methodology` + `.ai-footer-sources` in Type 2. |
...
|---|---|---|---|
| Next | Type 2 | concert-tax | Remove redundant `#ct-tabs` padding, swap `.ct-journey` HTML to `.ai-journey-compact` |
| Next | Type 2 | fast-food-sodium | Add `.ai-journey-compact` HTML, convert footer `<details>` to `.ai-footer-methodology` + `.ai-footer-sources` |
| TBD | Type 2 | gay-uncles, naming/* | Audit + add journey compact, confirm footer pattern |
| TBD | Batch B | Platform | OG tag audit, share block audit, breadcrumb audit |
